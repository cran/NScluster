...
# ty : the variable for the standardized coordinates of points in the rectangular region
...
# parameter
  ...
  if (plot == TRUE) {
    ...
    param <- xx[1:n, 1:it2]
    stddev <- z[[4L]][1:it2]
    plot.Simplex("T", stddev, param, pname)
  }

  if (process==0 || process==2)
  SimplexThomas.out <- list(pa.init=pa, logl.palm=z[[1L]][1:nip], mple=mples, logl.simplex=f, stderr=std, pa.normal=para)

  if (process==1 || process==3)
  SimplexThomas.out <- list(pa.init=pa, logl.palm=z[[1L]][1:nip], mple=mples, logl.simplex=f, stderr=std, pa.normal=para, ipri=ipri)
...
# parameter
  ...
  if (plot == TRUE) {
    ...
    param <- xx[1:n, 1:it2]
    stddev <- z[[4L]][1:it2]
    plot.Simplex("IP", stddev, param, pname)
  }

  if (process==0 || process==2)
  SimplexIP.out <- list(pa.init=pa, logl.palm=z[[1L]][1:nip], mple=mples, logl.simplex=f, stderr=std, pa.normal=para)

  if (process==1 || process==3)
  SimplexIP.out <- list(pa.init=pa, logl.palm=z[[1L]][1:nip], mple=mples, logl.simplex=f, stderr=std, pa.normal=para, ipri=ipri)
...
# parameter
  ...
  if (plot == TRUE) {
    ...
    param <- xx[1:n, 1:it2]
    stddev <- z[[4L]][1:it2]
    plot.Simplex("A", stddev, param, pname)
  }

  if (process==0 || process==2)
  SimplexTypeA.out <- list(pa.init=pa, logl.palm=z[[1L]][1:nip], mple=mples, logl.simplex=f, stderr=std, pa.normal=para)

  if (process==1 || process==3)
  SimplexTypeA.out <- list(pa.init=pa, logl.palm=z[[1L]][1:nip], mple=mples, logl.simplex=f, stderr=std, pa.normal=para, ipri=ipri)
...
# parameter
  ...
  if (plot == TRUE) {
    ...
    param <- xx[1:n, 1:it2]
    stddev <- z[[4L]][1:it2]
    plot.Simplex("B", stddev, param, pname)
  }

  pini <- c(pa[1]+pa[2], pa[3], pa[1]/(pa[1]+pa[2]), pa[4], pa[5])

  if (process==0 || process==2)
  SimplexTypeB.out <- list(pa.init=pini, logl.palm=z[[1L]][1:nip], mple=mples, logl.simplex=f, stderr=std, pa.normal=para)

  if (process==1 || process==3)
  SimplexTypeB.out <- list(pa.init=pini, logl.palm=z[[1L]][1:nip], mple=mples, logl.simplex=f, stderr=std, pa.normal=para, ipri=ipri)
...
#  mple <- alam, anu, a, sigma1, sigma2
  ...
  if (plot == TRUE) {
    ...
    param <- xx[1:n, 1:it2]
    stddev <- z[[4L]][1:it2]
    plot.Simplex("C", stddev, param, pname)
  }
  ...
  if (process==0 || process==2)
  SimplexTypeC.out <- list(pa.init=pini, logl.palm=z[[1L]][1:nip], mple=mples, logl.simplex=f, stderr=std, pa.normal=para)

  if (process==1 || process==3)
  SimplexTypeC.out <- list(pa.init=pini, logl.palm=z[[1L]][1:nip], mple=mples, logl.simplex=f, stderr=std, pa.normal=para, ipri=ipri)
...
print.Simplex <- function(x, ...) {
  ...
  if (ipri2 > 1) {
    ...
    cat("\n") 
    for( j in 1:ipri2 ) {
      if (x$ipri[j] == -1) cat(sprintf(" testfn = %18.10e", x$logl.palm[j]))
      if (x$ipri[j] == 1)  cat(sprintf(" update = %18.10e", x$logl.palm[j]))
  ...
  if (it2 > 1) {
    ...
    cat("\n")
    for (j in 1:it2) {
      cat(sprintf(" %i\t %15.3f\t %12.4e", j-1, x$logl.simplex[j], x$stderr[j]))
...
plot.Simplex <- function(type, std, para, pname) {
  old.par <- par(no.readonly=TRUE)
    new.mai <- old.par$mai
  ...
  n <- dim(para)[1]
  plot(std, type="l", ylim=c(0,2), xlab="Number of replications", ylab="The values of the standard deviations (SD) and \nthe normalized parameters")

  if (type == "T") mtitle <- "Thomas model"
  if (type == "IP") mtitle <- "Inverse-power type model"
  if (type == "A") mtitle <- "Type A model"
  if (type == "B") mtitle <- "Type B model"
  if (type == "C") mtitle <- "Type C model"

  for (i in 1:n) {
    par(new=TRUE)
    plot(para[i,], type="l", ylim=c(0,2), main=mtitle, ylab="", xlab="", col=i+1)

  }
  n1 <- n + 1
  legend("topright", legend = c("SD",pname), col=c(1:n1), lty=1, cex=0.8)
